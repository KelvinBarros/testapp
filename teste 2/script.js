...
const ui = {
  ...
  ulSeconds: $("#ulSeconds"),
  sizeMB: $("#sizeMB"),
  dlEndpoint: $("#dlEndpoint"),
  ulEndpoint: $("#ulEndpoint"),
...
function setMetric(el, bar, value, unit) {
  ...
  const v = Math.min(
    100,
    unit === "Mbps" ? (value / 200) * 100 : 100 - Math.min(100, value)
  ); // barras referenciais
  ...
  for (let i = 0; i < samples && running; i++) {
    try {
      ...
      const res = await fetch(
        url + (url.includes("?") ? "&" : "?") + "r=" + Math.random(),
      ...
      setMetric(
        ui.jitter,
        ui.jitterBar,
        times.length > 1 ? stdev(times) : 0,
        "ms"
      );
    } catch (err) {
      if (controller.signal.aborted) throw new Error("Ping abortado");
      log(`Falha ping #${i + 1}: ${err.message}`, "warn");
    }
  }
  const m = times.length ? mean(times) : null;
  const j = times.length > 1 ? stdev(times) : null;
  log(
    `Ping concluído. média=${m ? round(m) : "—"} ms, jitter=${
      j ? round(j) : "—"
    } ms`
  );
...
async function testDownload(baseUrl, seconds, threads, sizeMB) {
  log(
    `Iniciando download por ${seconds}s, ${threads} conexões, ~${sizeMB}MB por requisição`
  );
  ...
  // Controle para suavizar os números
  const history = [];
  let lastUpdate = 0;
      ...
      try {
        ...
        while (true) {
          ...
          if (elapsed > 0) {
            ...
            history.push(speed);
            if (history.length > 5) history.shift(); // mantém últimos 5 valores

            const now = Date.now();
            if (now - lastUpdate > 150) {
              // só atualiza a cada 0.5s
              const avg = history.reduce((a, b) => a + b, 0) / history.length;
              setMetric(ui.down, ui.downBar, formatMbps(avg), "Mbps");
              lastUpdate = now;
            }
          }
        }
        ...
        log(`Download[${id}] erro: ${err.message}`, "warn");
      } finally {
        active--;
      }
    }
  ...
  log(
    `Download total: ${(totalBytes / 1_000_000).toFixed(
      1
    )} MB em ${duration}s ≈ ${round(mbps, 1)} Mbps`
  );
  return { mbps };
}
...
async function testUpload(url, seconds, threads, sizeMB) {
  log(
    `Iniciando upload por ${seconds}s, ${threads} conexões, ~${sizeMB}MB por requisição`
  );
  ...
  async function worker(id) {
    ...
    while (running && performance.now() < endAt) {
      ...
      try {
        ...
        try {
          await res.arrayBuffer(); // garante que a conexão fecha corretamente
        } catch {}
        ...
        if (elapsed > 0) {
          const speed = size / elapsed; // bytes/s
          history.push(speed);
          ...
          if (now - lastUpdate > 500) {
            const avg = history.reduce((a, b) => a + b, 0) / history.length;
            setMetric(ui.up, ui.upBar, formatMbps(avg), "Mbps");
            lastUpdate = now;
          }
        }
      } catch (err) {
        if (controller.signal.aborted) break;
        log(`Upload[${id}] erro: ${err.message}`, "warn");
      }
    }
  }
  ...
  const mbps = formatMbps(totalBytes / duration);
  log(
    `Upload total: ${(totalBytes / 1_000_000).toFixed(
      1
    )} MB em ${duration}s ≈ ${round(mbps, 1)} Mbps`
  );
  return { mbps };
...
async function runTest() {
  ...
  // limites de segurança para evitar alocar Blobs gigantes que travam o browser
  // (você pode ajustar esses limites conforme necessário)
  const SAFE_MAX_UPLOAD_MB = 8;   // máximo por requisição
  const safeUploadSizeMB = Math.min(cfg.sizeMB, SAFE_MAX_UPLOAD_MB);
  ...
  try {
    ...
    // 3) Upload: antes de rodar, faz um probe rápido para detectar bloqueio CORS/endpoint
    if (!running) throw new Error("Interrompido");
    if (!cfg.ulEndpoint) {
      throw new Error("Endpoint de upload não configurado.");
    }

    // probe: tenta um POST pequeno em modo cors; se falhar, tenta no-cors como fallback
    let uploadProbeOk = false;
    try {
      const probeController = new AbortController();
      // corpo pequeno (1KB) só para verificar se o servidor aceita POST/CORS
      const probeBlob = new Blob([new Uint8Array(1024)], { type: "application/octet-stream" });
      await fetch(cfg.ulEndpoint + (cfg.ulEndpoint.includes("?") ? "&" : "?") + "r=" + Math.random(), {
        method: "POST",
        body: probeBlob,
        mode: "cors",
        cache: "no-store",
      ...
      log("Probe de upload falhou (CORS/endpoint). Tentando fallback 'no-cors'...", "warn");
      try {
        // fallback: envia como no-cors — ainda pode não ser legível, mas testa se o envio é permitido
        const probeBlob = new Blob([new Uint8Array(1024)], { type: "application/octet-stream" });
        await fetch(cfg.ulEndpoint + (cfg.ulEndpoint.includes("?") ? "&" : "?") + "r=" + Math.random(), {
          method: "POST",
          body: probeBlob,
        ...
        uploadProbeOk = true;
        log("Probe 'no-cors' concluído (resposta opaca). Se quiser ler resposta, configure CORS no servidor.", "warn");
      } catch (probeErr2) {
        uploadProbeOk = false;
        log("Probe de upload definitivamente falhou: " + probeErr2.message, "error");
      }
    }

    if (!uploadProbeOk) {
      throw new Error("Endpoint de upload indisponível ou bloqueado por CORS. Verifique o endpoint ou use um endpoint que aceite POST com CORS.");
    }

    // agora executa o upload com tamanho limitado por requisição (safeUploadSizeMB)
    ...
    log("Erro no teste: " + (err && err.message ? err.message : err), "error");
...
function stopTest() {
  ...
  running = false;
  abortAll();
  ui.status.textContent = "Interrompido";
...
async function copyResults() {
  ...
  try {
    ...
    log("Falha ao copiar: " + err.message, "warn");
  }
}
...
[ui.downBar, ui.upBar, ui.pingBar, ui.jitterBar].forEach(
  (el) => (el.style.transition = "width .25s ease")
);
...
if (navigator.connection) {
  ...
  if (["slow-2g", "2g", "3g"].includes(type)) {
    ui.threads.value = 2;
    ui.sizeMB.value = 2;
    ui.dlSeconds.value = 6;
    ui.ulSeconds.value = 6;
    log(
      `Rede detectada: ${type}. Ajustando parâmetros para evitar consumo exagerado.`,
      "warn"
    );
  }
}
